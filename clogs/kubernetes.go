...
func getAllPods(client kubernetes.Interface, namespace string) (*corev1.PodList, error) {
	pods, err := client.CoreV1().Pods(namespace).List(metav1.ListOptions{FieldSelector: "status.phase=Running"})
	if err != nil {
		return pods, err
	}
...
func (r *Clogs) matchPods(pods *corev1.PodList) (corev1.PodList, error) {
